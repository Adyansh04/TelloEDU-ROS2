...
class FaceTrackingNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('desired_contour_area', 70000)
        ...
        self.get_logger().info("Face Tracking Node Initialized")
    ...
    def yolo_callback(self, msg):
        if not msg.class_ids:
            # If no objects detected, sweep in angular z
            self.sweep()
            return

        # Sort indices based on contour area in descending order
        target_index = max(range(len(msg.class_ids)), key=lambda i: msg.contour_area[i])
        ...
        # self.get_logger().info(f"Errors: Area: {error_area}, X: {error_x}, Z: {error_z}")
        ...
        self.get_logger().warn(f"Controls: Area: {control_area}, X: {control_x}, Z: {control_z}")
...
def main(args=None):
    ...
    face_tracking = FaceTrackingNode()
    rclpy.spin(face_tracking)
    face_tracking.destroy_node()
    rclpy.shutdown()
