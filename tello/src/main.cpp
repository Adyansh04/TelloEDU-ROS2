...
class TelloControl : public rclcpp::Node
{
	public:
		...
		TelloControl() : Node("control")
		{
			...
			publisher_flip = this->create_publisher<std_msgs::msg::String>("flip", 10);
		...
		void manualControl(int key)
		{
			...
			if(key == (int)('w')) {msg.linear.z = manual_speed; RCLCPP_INFO(this->get_logger(), "Up");}
		...
		void timerCallback()
		{
			...
			if (key != NO_KEY)
			{
				...
				if(key == (int)('t'))
				{
					RCLCPP_INFO(this->get_logger(), "Taking off command received");
					publisher_takeoff->publish(std_msgs::msg::Empty());
					RCLCPP_INFO(this->get_logger(), "Taking off published");                   
				}
				// Land
				else if(key == (int)('l'))
				{
					RCLCPP_INFO(this->get_logger(), "Landing command received");
					publisher_land->publish(std_msgs::msg::Empty());
                    //Log
                    RCLCPP_INFO(this->get_logger(), "Landing published");
				}
				// Flip
				else if(key == (int)('f'))
				{
					RCLCPP_INFO(this->get_logger(), "Flip command received");
					std_msgs::msg::String msg = std_msgs::msg::String();
					msg.data = 'f';
					publisher_flip->publish(msg);
                    RCLCPP_INFO(this->get_logger(), "Flipping published");
				}
				// Emergency Stop
				else if(key == (int)('e'))
				{
                    ...
                    RCLCPP_INFO(this->get_logger(), "Emergency Stop published");
				}
				else
				{
					manualControl(key);
